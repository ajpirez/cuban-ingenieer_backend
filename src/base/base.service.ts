      ...
      try {
        ...
        if (relations.length > 0) {
          relations.forEach((relation: string) => {
            ...
            let currentAlias = 'entity'; // Start with the base alias
            let previousAlias = 'entity';

            nestedRelations.forEach((relationPart: string, index: number) => {
              const currentRelation = `${previousAlias}.${relationPart}`;
              currentAlias = relationPart;

              if (index === 0) {
                queryBuilder.leftJoinAndSelect(currentRelation, currentAlias);
              } else {
                queryBuilder.leftJoinAndSelect(currentRelation, currentAlias);
              }
